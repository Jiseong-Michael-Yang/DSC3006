...
# Check
30 == len(nominal_features) + len(numerical_features)
...
neigh.fit(x_train, y_train_result_only)

# Make predictions
y_pred = pd.DataFrame(neigh.predict(x_test))
y_pred.columns = ["Category"]

# Make submission files
answer = pd.concat([x_test["id"], y_pred], axis=1)
answer.to_csv("kNN_result.csv", index=None)
