...
dt.fit(x_train, y_train_result_only)
...
# Visualization
dot_data = export_graphviz(dt, out_file=None, feature_names=x_train.columns, class_names=str(np.unique(y_pred["Category"])))
graph = graphviz.Source(dot_data)
graph

# Make submission files
answer = pd.concat([x_test["id"], y_pred], axis=1)
answer.to_csv("DT_result.csv", index=None)
